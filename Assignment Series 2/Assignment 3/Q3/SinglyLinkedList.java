...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    ...
    public int count(E fromElement, E toElement) {
        int indStart = find1(fromElement, head, 0);
        int indStop = find2(toElement, head, indStart);

        return indStop+1;
    }

    public int find1(E val, Node<E> h, int offset) {
        if (h == null) return 0;
        else if (offset > 0) return find1(val, h.next, offset-1);
        else if(val != null && val.equals(h.value)) return 0;
        else return find1(val, h.next, 0) + 1;
    }

    public int find2(E val, Node<E> h, int offset) {
        if (h == null) return -1;
        else if (offset > 0) return find2(val, h.next, offset-1);
        else if(val != null && val.equals(h.value)) return 0;
        else return find2(val, h.next, 0) + 1;
    }
